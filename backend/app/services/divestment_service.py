...
def create_divestment_service(db: Session, user: dict, request: DivestmentRequest, inv_id: int):
    ...
    if investment_model.quantity_remaining < request.quantity:
        raise HTTPException(status_code=400, detail="Cannot divest more than the remaining quantity.")
    if investment_model.date_invested > request.date_divested:
        raise HTTPException(status_code=400, detail="Cannot divest before the respective investment's date!")
    ...
    divestment = Divestment(
        **request.model_dump(),
        ...
        company_id=investment_model.company,
    ...
    try:
        ...
        db.rollback()
        raise HTTPException(status_code=500, detail=f"Error creating divestment: {str(e)}")

    return divestment


def update_divestment_service(db: Session, user: dict, divestment_request: DivestmentRequest, id: int):
    divestment = db.query(Divestment).filter(Divestment.id == id, Divestment.owner_id == user.get("id")).first()
    ...
    try:
        ...
        db.rollback()
        raise HTTPException(status_code=500, detail=f"Error updating divestment: {str(e)}")
...
def delete_divestment_service(db: Session, user: dict, id: int):
    ...
    if user_model:
        user_model.number_of_divestments -= 1
        user_model.total_divestment -= (divestment.unit_price * divestment.quantity)

    # Update quantity remaining for specific investment
    investment_model = db.query(Investment).filter(Investment.id == divestment.investment_id).first()
    if investment_model:
    ...
    try:
        ...
        db.rollback()
        raise HTTPException(status_code=500, detail=f"Error deleting divestment: {str(e)}")
